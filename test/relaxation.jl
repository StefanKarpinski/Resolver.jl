...
@testset "relaxation stability: the collapse keeps forbidden versions" begin
    ...
    for (name, prob) in sources
        ...
        @test resolve_asis(info, σ) == resolve(data, [:A]; diagnose = false)
    end
...
@testset "relaxation stability: below column-closure it fails" begin
    ...
    @test resolve(data, σprob; diagnose = false) == Dict(:A => :v2) # raw: satisfiable
    ...
    @test resolve(data, σ2; diagnose = false) == Dict(:A => :v2)
