...
@testset "diagnose: requirement-level conflicts" begin
    ...
    for c in d2.conflicts
        @test resolve(data2, c.reqs; diagnose = false) === nothing
        for p in c.reqs
            @test resolve(data2, filter(≠(p), c.reqs); diagnose = false) !== nothing
        end
    end
end
...
@testset "diagnose: closure sub-instances" begin
    ...
    @test resolve(sub, ["A"]; diagnose = false) == resolve(info, ["A"]; diagnose = false)
    ...
    @test resolve(sub2, ["A"]; diagnose = false) == Dict("A" => 1, "B" => 1)
end
