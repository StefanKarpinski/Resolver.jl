...
@testset "workspaces" begin
    ...
    # manifest. This needs >= 1.12 for the feature, and -- since we generate a
    # full manifest -- a released Julia, so the resolution target matches the
    ...
    # a workspace whose root and one member are both packages, each with a
    ...
    run(`$julia --project=$project -e 'import Pkg; Pkg.instantiate()'`)
    out = IOBuffer()
    @test success(pipeline(`$julia --project=$project $script $ws --print-manifest --julia=$julia_version`; stdout=out))
    manifest = String(take!(out))
    # both workspace packages are recorded as path entries, the root as "."
    # and the member relative to it
    @test occursin(r"(?m)^\[\[deps\.WorkspaceRoot\]\]", manifest)
    @test occursin(r"(?m)^\[\[deps\.WorkspaceMember\]\]", manifest)
    @test occursin("path = \".\"", manifest)
    @test occursin("path = \"member\"", manifest)
    # and both members' registry dependencies are resolved into the manifest
    @test occursin(r"(?m)^\[\[deps\.Example\]\]", manifest)
    @test occursin(r"(?m)^\[\[deps\.JSON\]\]", manifest)
end
