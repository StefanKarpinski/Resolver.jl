    ...
    for m = 1:3, n = 1:3
        ...
        for _ = 1:100
            ...
            for reqs_bits = 0:2^m-1, by in (hi, lo)
                ...
                @test resolve(data, reqs; by, diagnose = false) == ref_resolve(data, reqs; by)
            end
        end
    end
end
...
@testset "consistency validation" begin
    ...
    sol = resolve(data, [:PkgA]; diagnose = false)
...
@testset "zero-version packages" begin
    ...
    @test resolve(info, [:A]; group = false, diagnose = false) === nothing
    data = Dict(
        :A => PkgData([:v2, :v1], Dict(:v2 => [:B]), nocomp),
        :B => PkgData(Symbol[], nodeps, nocomp),
        :C => PkgData([:v1], Dict(:v1 => [:A]), nocomp),
