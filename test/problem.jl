...
@testset "Problem: construction & masks" begin
    ...
    # one namespace, keyed by kind, whose value type is the closed union
    @test prob isa Problem{Symbol}
    @test sort!(collect(keys(prob.constraints))) == [:compat, :pin]
    @test valtype(prob.constraints) == Constraint{Symbol,Symbol,Vector{Symbol}}
    @test prob.constraints[:compat] isa Compat{Symbol,Vector{Symbol}}
    @test prob.constraints[:pin] isa Pins{Symbol,Symbol}
...
@testset "Problem: every keyword is a kind, and its shape is checked" begin
    ...
    @test prob.constraints[:pre] isa Predicate{Symbol}
...
@testset "Problem: a constraint answers three questions" begin
    ...
    for (_, c) in prob.constraints
        @test relax(c, named(c)) === nothing
        @test relax(c, Set{Symbol}()) !== nothing
    end
    compat = prob.constraints[:compat]
    @test named(compat) == Set([:A])
    @test excludes(compat, :A, :v2) && !excludes(compat, :A, :v1)
    @test !excludes(compat, :Z, :v2) # a package it doesn't name
    pin = prob.constraints[:pin]
    @test named(pin) == Set([:B])
    @test excludes(pin, :B, :v1) && !excludes(pin, :B, :v2)
    pre = prob.constraints[:pre]
    @test named(pre) === nothing # a predicate cannot enumerate them
    @test excludes(pre, :Z, :v3) && excludes(pre, :Y, :v3)
    # ... but it can still be relaxed for a package, like anything else
    @test !excludes(relax(pre, [:Z]), :Z, :v3)
    @test  excludes(relax(pre, [:Z]), :Y, :v3)
end
...
@testset "Problem: the unconstrained case is free" begin
    ...
    @test p.constraints[:compat].allowed == d
    @test p.constraints[:compat].allowed !== d
