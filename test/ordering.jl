...
function test_order_equivalence(data, prob, order; by::Function = identity)
    ...
    for group in (true, false)
        @test resolve(data, prob; by, order, group, diagnose = false) ==
              resolve(baked, prob; by, group, diagnose = false)
    end
end
...
@testset "ordering: permutations and identity" begin
    ...
    @test resolve(data, [:A, :B]; order = up, diagnose = false) == Dict(:A => :v1, :B => :v1)
end
        ...
        for _ = 1:60
            ...
            for mk in (reversed, shuffled, partly)
                ...
                @test resolve(data, prob; by, order, diagnose = false) ==
                      resolve(bake(bake_order(data, order), prob), reqs; by, diagnose = false)
            end
        end
    end
end
        ...
        for _ = 1:20
            ...
            for reqs_bits = 1:2^m-1
                ...
                for prob in (Problem(reqs), Problem(reqs; compat, pins)),
                    ...
                    baked = bake_order(data, order)
                    @test resolve(all_reqs, prob; order, diagnose = false) == resolve(baked, prob; diagnose = false)
