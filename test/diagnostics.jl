...
#   * for the fixes, a resolve. A fix claims that carrying out its actions
#     yields its solution, so the actions are applied to the `Problem` here —
#     independently of how the diagnosis applies them — and the modified
#     problem is resolved from the artifact.
#
# What the report says is checked separately, against hand-built cases, since
# there is no oracle for English.

using Resolver: Problem, PkgData, PkgInfo, SAT, Diagnosis, pkg_info, NoKinds,
    prepare_pkg_info, finalize, is_satisfiable, installed_lit, forbidden_lit,
    with_classes_relaxed, class_exclusions, exclusion_sources, nclasses,
    sat_assume_var
...
# `random_constraints` (problem.jl) and `verdicts` (relaxation.jl) are the
# generator and the instance-comparison battery those files already carry;
# both files run before this one.

## helpers

# the instance a resolve of `prob` fails on, and the artifact behind it
function failed_instance(data, prob; order = nothing)
    info = pkg_info(data, prob)
    sat = SAT(prepare_pkg_info(info, prob; order))
    return sat, info
end
...
# `prob` with `actions` carried out, built here rather than taken from the
# diagnosis: the requirements it names dropped, and every constraint source it
# names taken off the package it names
function apply_actions(prob::Problem{P}, actions::Vector{Action{P}}) where {P}
    reqs = P[p for p in prob.reqs if Action{P}(:drop, p) ∉ actions]
    compat = Dict{P,Any}(k => v for (k, v) in prob.compat
                         if Action{P}(:compat, k) ∉ actions)
    pins = Dict{P,Any}(k => v for (k, v) in prob.pins
                       if Action{P}(:unpin, k) ∉ actions)
    excludes = Pair{Symbol,Any}[
        kind => (p, v) -> Action{P}(kind, p) ∉ actions && forbids(p, v)::Bool
...
function check_diagnosis(data, prob::Problem{P}; order = nothing,
    ...
    sat, info = failed_instance(data, prob; order)
    try
        ## the instance is left as it was found
        #
        # `diagnose` pops the deactivation frame and pushes a frame of
        # definitional clauses inside it, so run it here too and check that all
        # of that came back — the checks below, and anything else that reuses
        # an instance, are entitled to find it as they left it. The battery is
        # one solve per class, so on a large universe the verdict on the failed
        # query is what is affordable
        battery = sum(nclasses(i) for i in values(sat.info); init = 0) ≤ 64
        before = battery ? verdicts(sat) : Bool[]
        deact = copy(sat.deact)
        @test Diagnostics.diagnose(sat, prob, info; by, order) isa Diagnosis
        ...
        for c in d.conflicts, f in c.chain
            ...
            if haskey(sat.info, f.pkg)
                ...
                @test f.excluded ==
                    [exclusion_sources(prob, f.pkg, v) for v in f.members]
        ...
        for fix in d.fixes
            ...
            # exactly the solution the fix shows
            sol = resolve(info, apply_actions(prob, fix.actions);
                order, by, diagnose = false)
            @test sol == fix.solution
        end
...
@testset "diagnosis: the story names the constraint" begin
    prob = Problem([:A]; compat = Dict(:B => [:w1]), pins = Dict(:B => :w2))
    ...
    # the cheapest version to give back costs one source, so that is the fix —
...
@testset "diagnosis: an exclusion kind is a constraint like any other" begin
    prob = Problem([:A];
        excludes = [:prerelease => (p, v) -> p === :B && v !== :w1,
                    :yanked => (p, v) -> p === :B && v === :w1])
    d = check_diagnosis(needs_dep, prob)
    f = d.conflicts[1].chain[2]::Availability
    @test f.excluded == [[:prerelease], [:prerelease], [:yanked]]
    # one source is enough to give the package back, and the best version it
...
@testset "diagnosis: the instance is left as it was found" begin
    ...
    # frame comes off, a frame of definitional clauses goes on inside it, and
    # both are undone. An instance is reusable, so what has to be true
    # afterwards is that it answers every question exactly as it did — the
    # emptied classes still emptied, every verdict what it was.
    #
    ...
    unavailable_dep = Problem([:A];
        compat = Dict(:B => [:w1]), pins = Dict(:B => :w2))
    ...
    for (data, prob) in cases
        sat, info = failed_instance(data, prob)
        try
            before = verdicts(sat)
            deact = copy(sat.deact)
            # diagnosing twice over, since restoring has to be repeatable
            for _ = 1:2
                @test Diagnostics.diagnose(sat, prob, info) isa Diagnosis
...
@testset "diagnosis: what resolve returns" begin
    ...
    for src in (needs_dep, pkg_info(needs_dep, prob))
        ...
        # ... through the convenience keywords too
        @test resolve(src, [:A]; compat = Dict(:B => Symbol[])) isa Diagnosis
        @test resolve(src, [:A]; compat = Dict(:B => Symbol[]),
    ...
    # the artifact comes through a diagnosed resolve intact, since the fixes
    # are resolved against it
    info = pkg_info(needs_dep, prob)
    before = deepcopy(info)
    @test resolve(info, prob) isa Diagnosis
    @test info == before
    # ... including one the resolve is otherwise allowed to consume
...
@testset "diagnosis: the report" begin
    ...
    # every source that excludes a version is named, and a long line is filled
    d = resolve(needs_dep,
        Problem([:A]; compat = Dict(:B => [:w1]), pins = Dict(:B => :w2)))
    @test sprint(show, MIME("text/plain"), d) == """
        Unsatisfiable — 1 conflict, 2 fixes:
...
@testset "diagnosis: the report says nothing about how it was found" begin
    ...
    cases = [
        ...
        needs_dep => Problem([:A]; pins = Dict(:B => :w9)),
        needs_dep => Problem([:A]; compat = Dict(:B => Symbol[]),
                                   pins = Dict(:A => :v9)),
    ]
    for (data, prob) in cases
...
@testset "diagnosis: verified fixes over generated data" begin
    ...
    for (m, n) in ((2, 2), (2, 3), (3, 2), (3, 3), (2, 4), (4, 2))
        ...
        for _ = 1:10
            ...
            kinds = Pair{Symbol,Any}[:ban => (p, v) -> (p, v) in banned]
            for cs in (nothing, random_constraints(m, n)),
                excludes in (NoKinds, kinds)
