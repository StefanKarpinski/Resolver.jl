...
function resolves(reqs::Vector{UUID}; julia::VersionSpec)
    ...
    Resolver.resolve(info, prob; diagnose = false) !== nothing
end
...
@testset "bin/Registries.jl" begin
    ...
    @testset "project compat travels in the Problem" begin
        ...
        info = Resolver.pkg_info(reg, prob)
        sol = Resolver.resolve(info, prob; diagnose = false)
        @test sol !== nothing
    ...
    @testset "the version ordering is a resolve parameter" begin
        ...
        for lt in (<, minor, rminor)
            ...
            baked = Dict(u => Resolver.PkgData(
                         ...
                         for (u, d) in data)
            @test Resolver.resolve(data, prob; order, diagnose = false) ==
                  Resolver.resolve(baked, prob; diagnose = false)
            # ... and the one T1 artifact answers under any of them
            @test Resolver.resolve(info, prob; order, diagnose = false) ==
                  Resolver.resolve(baked, prob; diagnose = false)
        end
        ...
        @test Resolver.resolve(info, prob; order, diagnose = false) == Resolver.resolve(baked, prob; diagnose = false)
        @test Resolver.resolve(info, prob; order, diagnose = false)[JULIA_UUID] <
              Resolver.resolve(info, prob; diagnose = false)[JULIA_UUID]
    end
    ...
    @testset "prerelease admission is a resolve-time constraint" begin
        ...
        for allow_pre in (no_pre, all_pre,
            ...
            @test Resolver.resolve(data, prob; diagnose = false) == Resolver.resolve(old, reqs; diagnose = false)
            @test Resolver.resolve(info, prob; diagnose = false) == Resolver.resolve(old, reqs; diagnose = false)
        end
    ...
    @testset "yanked versions are pre-filtered from the universe" begin
        ...
        @test Resolver.resolve(raw, prob; diagnose = false) == Resolver.resolve(data, reqs; diagnose = false)
        ...
        @test Resolver.resolve(raw, both; diagnose = false) == Resolver.resolve(data, plain; diagnose = false)
        order = u -> (<)
        @test Resolver.resolve(raw, both; order, diagnose = false) ==
              Resolver.resolve(data, plain; order, diagnose = false)
    ...
    @testset "the julia universe is the whole universe" begin
        ...
        for julia in (VersionSpec("1.9"), VersionSpec("1.10"), VersionSpec("1.11"),
            ...
            @test sol == Resolver.resolve(reg, prob; diagnose = false)
        end
...
@testset "unsat diagnostics (bin/resolve.jl)" begin
    ...
    @test occursin("Unsatisfiable", report)
