...
@testset "resolver API" begin
    @testset "example: 0 pkgs" begin
        ...
        for (P, V) in [(String, VersionNumber), (Real, Complex{Int})]
            ...
            required = P[]
            resolved = Resolver.resolve(required, versions)
            @test resolved == [[]]
            @test eltype(resolved) == Vector{Pair{P,Union{}}}
        end
    end
    ...
    @testset "example: 2 pkgs, 1 conflicts" begin
        ...
        resolved = Resolver.resolve(required, versions; compat)
        @test resolved == [["A" => 1]]
    end
    ...
    @testset "example: 2 pkgs, 2 deps" begin
        ...
        required = ["A"]
        resolved = Resolver.resolve(required, versions; deps)
    ...
    @testset "example: 2 pkgs, 2 deps, 1 conflicts" begin
        ...
        required = ["A"]
        compat = gen_compat(conflicts)
        resolved = Resolver.resolve(required, versions; compat, deps)
        @test resolved == [
            ["A" => 1, "B" => 2],
            ["A" => 2, "B" => 1],
        ]
